...
## using shiny
# ```{r shiny_map, echo=FALSE}
# years <- sort(unique(shootings_sf$year))
# sexes <- unique(shootings_sf$sex)
# year_colors <- colorFactor("viridis", domain = years)

# ui <- fluidPage(
#   titlePanel("Interactive Shooting Map by Year and Sex"),
#   sidebarLayout(
#     sidebarPanel(
#       checkboxGroupInput("year_filter", "Select Year(s):", choices = years, selected = years),
#       checkboxGroupInput("sex_filter", "Select Sex(es):", choices = sexes, selected = sexes)
#     ),
#     mainPanel(
#       leafletOutput("shooting_map", height = 600)
#     )
#   )
# )
...
#       addLegend("bottomleft", pal = year_colors, values = shootings_sf$year, title = "Year")
#   })

#   observe({
#     req(input$year_filter, input$sex_filter)

#     filtered_data <- shootings_sf %>%
#       filter(year %in% input$year_filter, sex %in% input$sex_filter)

#     leafletProxy("shooting_map") %>%
#       clearMarkers() %>%
#       addCircleMarkers(
#         data = filtered_data,
#         color = ~year_colors(year),
