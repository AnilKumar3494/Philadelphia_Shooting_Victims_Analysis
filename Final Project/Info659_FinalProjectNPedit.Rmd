...
We will create a column that clarifies race, so that it can be used later in visualizations.
```{r}
shootings.adjusted$race_adjusted <- ifelse(shootings.adjusted$race == "A", "Asian", 
                        ifelse(shootings.adjusted$race == "B", "Black", 
                               ifelse(shootings.adjusted$race == "W", "White", "Unknown")))
...
Looking at the percent of total victims who are male and female. 8.6% are female and 91.4% are male.
```{r}
sex.percent <- prop.table(table(fatal.only$sex)) * 100

sex.percent
```

### Fatal Shooting Victim Latino Status
Looking at the percentage of fatal shooting victims who are latino. 87.6% are non-latino and 12.4% are latino.
```{r}
latino.percent <- prop.table(table(fatal.only$latino)) * 100
...
The racial profile of fatal shooting victims is best visualized as a pie chart.

```{r}
race.counts = fatal.only %>%
  count(race_adjusted)
...
```{r}
pct <- round(race.counts$n/sum(race.counts$n)*100)
lbls <- paste(race.counts$race_adjusted, pct) # adds percentages to labels
lbls <- paste(lbls,"%",sep="") # adds % sign
 colors = c('black','grey','blue')

pie <- pie(race.counts$n, lbls, main = "Fatal Shooting Victims by Race", col=colors)
...
Examing racial percentages of fatal shooting victims by year to see if there were annual changes in composition. The best visualization of this trend is a stacked bar chart.
```{r}
race.byyear <- fatal.only %>%
  group_by(year, race_adjusted) %>%
  summarize(Count = n())

race.byyear
```
```{r}
ggplot(race.byyear, aes(fill=race_adjusted, y=Count, x=year)) + 
    geom_bar(position="fill", stat="identity") +
  ggtitle("Racial Composition of Fatal Shooting Victims by Year") +
  labs(fill = "Race") +
  labs(y = "Fatality Percent") +
  labs(x = "Year")
```
```{r}
ggplot(race.byyear, aes(fill=race_adjusted, y=Count, x=year)) + 
...
The below geospatial analysis will provide heatmaps of where shootings occur across the city, both overall and over the years included in the data set.
...
```{r}
# --- R Chunk: Leaflet Map with Clustering and Enhanced Heatmaps ---
...
offender_deceased_sf <- shootings_sf_for_map %>%
  dplyr::filter(offender_deceased == 'Y') # 'Y' is typical for character flags
...
map_enhanced_heatmaps <- leaflet::leaflet(data = shootings_sf_for_map) %>% # Base data
  ...
  # Layer 1: Clustered Markers (as before)
  ...
  # Layer 2: Heatmap for All Incidents (existing)
  ...
  # Layer 3: Heatmap for All Fatal Incidents (existing - uses total_fatality)
  leaflet.extras::addHeatmap(
    data = dplyr::filter(shootings_sf_for_map, total_fatality > 0), 
    lng = ~sf::st_coordinates(geometry)[,1], 
    lat = ~sf::st_coordinates(geometry)[,2],
    blur = 20, max = 0.05, radius = 15,
    group = "Heatmap (All Fatal Incidents)" # Clarified name
  ) %>%

  
  # Layer 4: Heatmap for Officer Involved & Fatal Incidents
  leaflet.extras::addHeatmap(
  ...
  leaflet.extras::addHeatmap(
    ...
    lng = ~sf::st_coordinates(geometry)[,1], 
    lat = ~sf::st_coordinates(geometry)[,2],
    blur = 20, max = 0.05, radius = 15, # Adjust parameters if needed for this specific subset
    group = "Heatmap (Officer Involved & Non-Fatal)"
  ) %>%
  
  # Layer 5: Heatmap for Offender Deceased Incidents
  leaflet.extras::addHeatmap(
  ...
  # Layer 6: Heatmap for All Officer Involved Incidents
  leaflet.extras::addHeatmap(
  ...
  leaflet::addLayersControl(
    ...
    overlayGroups = c(
      ...
      "Heatmap (Officer Involved & Non-Fatal)", # Added new layer
...
Overall, the data reveals that the fatal shooting victim profile has remained steady from 2015 - 2024 despite notable changes in total fatal shootings. Most victims are black, non-latino males between the ages of 20-29. The most fatal type of shooting is one with multiple gunshot wounds. Examining the heat map of the city shows hot spots over the years and by shooting type. You can find clusters of shootings along particular blocks in particular neighborhoods by exploring the heat map.

The data is prone to reporting bias. The police can select how they choose to report it. As we noted, they do not give any demographics in officer-involved shootings. The demographic data can also be subjective. Someone who has been fatally shot will not be able to provide their racial identity or if they consider themselves to be latino, so someone is determining that on their behalf. There are only three options for race, making it impossible to include scenarios like being multiracial.  These limitations means that this data likely lacks some nuance.

This analysis could be useful to inform various interested parties in the city. The police would find the heat map useful for targeting law enforcement activity. Hospitals and social services would be helped by both the heat map and the victim profiles. Hospitals can plan to look for these high risk cases to intervene quickly. Social services can see who is most in need of preventative interventions.